...
while 1:
    ...
    print()
    print("Generated resource key: '{0}' → {1}".format(string_to_replace, resource_key))

    # check if the resource key is present in any of the resources files
    for resource_name, resource_content in resources_content.items():
        if '"' + resource_key + '"' in resource_content:
            # it exists
            # save the resource file path
            resource_name = get_file_name_from_path(resource_name)
            print()
    ...
    if resource_name is "":
        ...
        if 0 <= i_resource_chosen < len(resources):
            ...
            for language in languages:
                other_resource = resources[i_resource_chosen].replace(".resx", ".{0}.resx".format(language))
                ...
                print("Resource correctly added in the {0} resource file.".format(language))

            # save the resource file path
            resource_name = get_file_name_from_path(resources[i_resource_chosen])
        else:
            print("Error")
    print()
    ...
    if resource_name is not "":
        ...
        print("You can get this resource using one of this codes:")
        print()
